...
%% setpoint change with soft controller
...
for subbatch = 1:length(x_test) 
    ...
    for t = time_to_steady_state+1 : T
        ...
        Kd_output = B_kd(4)/2*Kd_input_array(t-nk_kd-3) + B_kd(5)/2*Kd_input_array(t-nk_kd-4) + B_kd(6)/2*Kd_input_array(t-nk_kd-5) ...
                    + white_noise_kd(t) + C_kd(2)/2*white_noise_kd(t-1) ...
                    - (A_kd(2)/2*Kd_output_array(t-1) + A_kd(3)/2*Kd_output_array(t-2) + A_kd(4)/2*Kd_output_array(t-3) + A_kd(5)/2*Kd_output_array(t-4));

        Kd_output_array(t) = Kd_output;

        % ----------- Kt: tension error -> furnace power  -------------
        Kt_input_array(t) = tension_error;
        Kt_output = B_kt(4)/2*Kt_input_array(t-nk_kt-3) + B_kt(5)/2*Kt_input_array(t-nk_kt-4) ...
                    + white_noise_kt(t) + C_kt(2)*white_noise_kt(t-1) + C_kt(3)*white_noise_kt(t-2) ...
                    - (A_kt(2)/2*Kt_output_array(t-1) + A_kt(3)/2*Kt_output_array(t-2) + A_kt(4)/2*Kt_output_array(t-3));
